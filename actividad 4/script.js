...
// La informacion debe ser guardada como objeto dentro del array users asignadole un id unico a cada registro.
 ...
 /*
let usuario = [ {nombre: "", apellido: "", edad: "", profesion: ""},]

 function Llenar(objeto){
    for (const index in objeto) {
           if (objeto [index] === "") {
           objeto [index] = prompt(`Ingrese ${index}`)
       }
    } return objeto
}
for(const index in usuario){
         Llenar(usuario[index])
 }
...
/*
let usuario1 = [ {nombre: "", apellido: "", edad: "", profesion: "", created_date: Date()},]
function Llenar(objeto){
    for (const index in objeto) {
           if (objeto [index] === "") {
           objeto [index] = prompt(`Ingrese ${index}`)
...
for(const index in usuario1){
         Llenar(usuario1[index])
 }
...
function sortusers(argument){
    let validInput = ["created_at"]

    if (validInput.includes(argument)){
        let result = argument === "created_at" ? (users.sort((a,b)=> a[argument].localeCompare(b[argument]))) : (users.sort((a,b)=> a[argument]- b[argument]))
        
        console.log(result)
    } else {
        console.log("Este campo no es posible de encontrar")
    }
}*/
...
//let fullName = prompt("Por favor ingresa: Nombre, Apellido, Edad y Profesión", "Nombre Apellido Edad Profesion")

//var myArray = fullName.split(" ")

//alert( "Nombre:" + myArray[0] + "Apellido:" + myArray[1] + "Edad:" + myArray[2] + "Profesion:" + myArray[3]) 

//document.getElementById("root").innerHTML = "Nombre:" + myArray[0] +"</br>"+ "Apellido:" + myArray[1] +"</br>"+ "Edad:" + myArray[2] +"</br>"+ "Profesion:" + myArray[3] 
...
do{
  //var cadena= prompt("introduce mensaje");
  var fullName = prompt("Por favor ingresa: Nombre, Apellido, Edad y Profesión", "Nombre Apellido Edad Profesion");
  if (texto== ""){
    texto = fullName.split(" ") ;
  }
  else{
    texto= "Nombre:" + texto[0] +"</br>"+ "Apellido:" + texto[1] +"</br>"+ "Edad:" + texto[2] +"</br>"+ "Profesion:" + texto[3] ;
  }
}
...
(function load(){
    ...
    $btnAdd.addEventListener("click", (event) => {
        if($form.name.value != "" && $form.lastName.value != "" && $form.Edad.value != "" && $form.Profesion.value != ""&& $form.Fecha.value != ""){
            ...
            $div.innerHTML = templateElement(`${$form.name.value} ${$form.lastName.value} ${$form.Edad.value} ${$form.Profesion.value} ${$form.Fecha.value}`, index)
